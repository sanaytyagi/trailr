...
import {
  ArrowLeft, Loader2, CheckCircle2, Bold, Italic, Underline,
...
interface Segment {
  text: string;
  commentId: string | null; // null = plain, "__pending__" = pending
  colorIndex: number;
}
...
/** Map a DOM (node, offset) inside `container` to a character position. */
...
/** Split `body` into segments based on active (non-resolved) comments + optional pending range. */
...
export default function EssayEditorPage({ params }: { params: Promise<{ essayId: string }> }) {
  ...
  useEffect(() => {
    if (!profile) return;
    async function load() {
      // Fetch without user_id filter — RLS already limits to own or student's essays
  ...
  function handleTextSelect() {
    ...
    // Block overlapping active comments
  ...
  async function resolveComment(commentId: string) {
    await supabase.from("essay_comments").update({ resolved: true }).eq("id", commentId);
    setComments((prev) => prev.map((c) => c.id === commentId ? { ...c, resolved: true } : c));
  }

  // Dismiss floating button on outside mousedown
  useEffect(() => {
  ...
  const toolbarBtn = "flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground hover:bg-muted hover:text-foreground transition-colors disabled:opacity-40";
        ...
        <div className="flex gap-6 items-start">
          ...
          <div className="flex-1 min-w-0 rounded-xl border border-border bg-card overflow-hidden shadow-sm">
            ...
            <div className="px-6 py-5 bg-muted/30 border-b border-border">
              <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground mb-1.5">Prompt</p>
            ...
            <div className="flex items-center justify-between px-6 py-3 border-t border-border bg-muted/10">
              <span className={cn("text-xs font-semibold tabular-nums", over ? "text-destructive" : "text-foreground")}>
                {words} / {essay.word_limit} words
              </span>
              <span className={cn("text-xs tabular-nums", over ? "text-destructive font-semibold" : "text-muted-foreground")}>
                {over ? `${words - essay.word_limit} over limit` : `${remaining} remaining`}
              </span>
            </div>
          </div>
            ...
            {activeComments.map((c) => {
              ...
              return (
                ...
                >
                  ...
                  <p
                    className="text-[11px] italic leading-snug mb-2 rounded px-1.5 py-0.5"
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 sm:px-6">
        ...
        <div className="flex gap-6 items-start">
          ...
          <div className="flex-1 min-w-0 rounded-xl border border-border bg-card overflow-hidden shadow-sm">
            ...
            <div
              ref={essayTextRef}
              className="px-6 py-5 text-sm leading-relaxed whitespace-pre-wrap min-h-[420px]"
            >
              {body.trim() === "" ? (
                <span className="text-muted-foreground italic">No content yet.</span>
              ) : (
